...
**PR4 후속(구현됨)** — `app/ocr/fieldHeuristics.ts`, 순수 TS, 테스트 12:
- **주소 heuristic**: 지역+장소유형+행정접미사(구/시/로/길/동/층/호)로 `scoreAddress`(0~1). 라벨 매칭 실패 시 최고점 주소 라인을 폴백 선택 → `서울 없는 '영등포구 가마산로…'` 같은 주소도 회수.
- **업체 vs 수령자 disambiguation**: `extractPersonName`이 역할 접두어(신부/상주/받는분) 제거 후 업체(화원/플라워/㈜)·주소·지시문·숫자를 배제해 2~4자 이름(+직함)만 인정 → 화원명이 수령자로 오배정되는 것을 차단.
- **앵커-값 재랭킹**: `pickBest`(범용 scorer)·`pickTypedValue`(타입 검증 기반) — confidence 대신 값의 적합도로 후보 선택.
- 서비스 통합: 수령자→`extractPersonName`, 주소 폴백→최고점 라인. 전체 204 테스트 그린.

**남은 작업:** 후보 병합(멀티패스, PR3와 연동), 업체명↔전화 쌍 검증, 학습 루프(PR6).
